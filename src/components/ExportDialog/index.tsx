    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
      <div className="bg-[#111922] border border-[#253347] rounded-lg w-96 p-5">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-sm text-[#c8daea] font-medium">{t('export.title', { table: tableName })}</h3>
          <button onClick={onClose} className="text-[#7a9bb8] hover:text-[#c8daea]"><X size={16}/></button>
        </div>
        <div className="space-y-3">
          <div>
            <label className="text-xs text-[#7a9bb8] mb-1 block">{t('export.format')}</label>
            <div className="flex gap-2">
              {(['csv', 'json', 'sql'] as const).map(f => (
                <button key={f} onClick={() => setFormat(f)}
                  className={`px-3 py-1 text-xs rounded ${format === f ? 'bg-[#3794ff] text-[#c8daea]' : 'bg-[#1a2639] text-[#7a9bb8]'}`}>
                  {f.toUpperCase()}
                </button>
              ))}
            </div>
          ...
          <div>
            <label className="text-xs text-[#7a9bb8] mb-1 block">WHERE ({t('export.optional')})</label>
            <input
              className="w-full bg-[#0d1520] border border-[#1e2d42] rounded px-2 py-1.5 text-xs text-[#c8daea] outline-none focus:border-[#009e84]"
              placeholder="id > 100"
              value={whereClause}
        ...
        <div className="flex justify-end gap-2 mt-5">
          <button onClick={onClose} className="px-3 py-1.5 bg-[#1a2639] text-[#7a9bb8] text-xs rounded">{t('common.cancel')}</button>
