...
export function QueryHistory() {
  ...
  if (!activeConnectionId) {
    return <div className="p-4 text-[#858585] text-sm">{t('queryHistory.selectConnectionFirst')}</div>;
  }

  return (
    <div className="h-full overflow-auto">
      {queryHistory.length === 0 ? (
        <div className="p-4 text-[#858585] text-sm">{t('queryHistory.noQueryHistory')}</div>
      ) : (
        queryHistory.map((h) => (
          <div
            key={h.id}
