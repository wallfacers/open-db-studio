  ...
  return (
    ...
    >
      <div className="flex flex-col gap-4">
        {/* 连接选择 */}
        <div>
          <label className="block text-xs text-[var(--foreground-muted)] mb-1">{t('erDesigner.connectionLabel')}</label>
          <DropdownSelect
        ...
        {/* 数据库选择 */}
        <div>
          <label className="block text-xs text-[var(--foreground-muted)] mb-1">{t('erDesigner.databaseLabel')}</label>
          {loadingDatabases ? (
        ...
        {showSchema && (
          <div>
            <label className="block text-xs text-[var(--foreground-muted)] mb-1">{t('erDesigner.schemaLabel')}</label>
            {loadingSchemas ? (
              <div className="text-xs text-[var(--foreground-muted)]">{t('common.loading')}</div>
            ) : (
              <DropdownSelect
