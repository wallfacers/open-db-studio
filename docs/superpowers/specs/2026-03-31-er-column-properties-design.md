...
**`data_type` 字段只存基础类型名**（如 `"VARCHAR"`），长度/精度拆分到独立的 `length`/`scale` 字段。

理由：结构化存储便于类型注册表查询、兼容性检查、UI 编辑控件的独立绑定。避免前端解析字符串的脆弱性。

**Rust DDL 生成器适配**：现有 `ddl_generator.rs` 的 `parse_type()` 函数从 `data_type` 字符串解析长度。迁移后改为直接读取 `length`/`scale` 字段拼接，`parse_type()` 可移除。各方言 `map_type()` 实现入参从 `&str` 改为 `(base_type: &str, length: Option<i32>, scale: Option<i32>)`。

**向后兼容**：已有数据中 `data_type` 可能含 `"VARCHAR(255)"` 格式。SQLite 迁移脚本需在添加新列后，执行一次性数据清洗：解析已有 `data_type` 中的括号部分，填入 `length`/`scale`，并将 `data_type` 截断为基础类型名。
...
interface ErColumn {
  ...
  table_id: number
  name: string
  data_type: string             // 只存基础类型名，如 "VARCHAR"
  nullable: boolean
...
默认值: [____________]
ENUM 值: [active, inactive] [编辑...]   ← 仅 hasEnumValues 类型显示
...
ENUM 值编辑器：类型为 ENUM/SET 时显示。点击「编辑...」打开内联编辑区域，每行一个值，支持增删排序。
...
仅 `ERTableNode.tsx`，改动量中等。

### 变化

类型显示从 `VARCHAR` 变为 `VARCHAR(255)`。下拉选项从硬编码数组改为引用类型注册表。类型切换时需根据 DataTypeDefinition 自动填入 defaultLength/defaultScale 或清空。
...
这是对侧边栏中**列区域的完整重写**，从当前的单行 ColumnRow 列表改为表格布局。侧边栏的项目树结构、表展开折叠等不变。
...
- `< 450px`：隐藏注释列
- `< 380px`：隐藏默认值列
- `< 300px`：隐藏 UQ 列，最小保留列名 + 类型 + NN + ⋮

使用 `ResizeObserver` 监听侧边栏容器宽度，通过 state 控制列可见性。
...
drawerFocusColumnId: number | null  // 打开时自动展开并滚动到该列
openDrawer(tableId: number, focusColumnId?: number): void
closeDrawer(): void
```

`openDrawer` 的 `focusColumnId` 参数用于从侧边栏 ⋮ 菜单「在抽屉中编辑」入口。ColumnsTab 渲染时检查 `drawerFocusColumnId`，如非 null 则自动展开对应列并 scrollIntoView，展开后将 `drawerFocusColumnId` 清为 null。
...
- 解绑连接时：清空全部 `dialectWarnings`
- 字段类型改为兼容类型时：移除该 columnId 的警告
- 删除字段时：移除该 columnId 的警告
- 切换绑定方言时：重新全量扫描，替换整个 `dialectWarnings`
...
checkDialectCompatibility(): void        // 全量扫描
checkColumnCompatibility(columnId: number): void  // 单列检查
clearDialectWarnings(): void             // 清空所有警告
```
...
| 文件 | 变更 |
|------|------|
| `src-tauri/src/er/models.rs` | `ErColumn` struct 新增 8 个字段 |
| `src-tauri/src/er/models.rs` | `CreateColumnRequest`、`UpdateColumnRequest` 新增对应字段 |
| `src-tauri/src/er/repository.rs` | INSERT/UPDATE/SELECT 语句扩展新列 |
| `src-tauri/src/er/ddl_generator.rs` | `parse_type()` 移除，改为直接读 length/scale；各方言 `map_type()` 入参从 `&str` 改为 `(base_type, length, scale)` |
| `schema/init.sql` | er_columns 表定义新增列 |
| `src-tauri/src/db/migrations.rs`（或启动逻辑） | 自动迁移逻辑 |
...
**数据清洗**：迁移脚本在添加新列后，遍历已有 `data_type` 含括号的记录（如 `"VARCHAR(255)"`），解析出 length/scale 填入新字段，并将 `data_type` 截断为基础类型名（如 `"VARCHAR"`）。

### DDL 生成

各方言 `map_type()` 直接接收 `(base_type, length, scale)` 参数拼接输出。利用新字段生成完整 DDL：`VARCHAR(255)`、`DECIMAL(10,2)`、`NOT NULL`、`UNIQUE`、`DEFAULT`、`UNSIGNED`、`COMMENT`、`ENUM()`。

### 数据库同步（syncFromDatabase）

从真实数据库导入列时，需解析数据库返回的类型信息（如 `varchar(255)`），拆分为 `data_type="VARCHAR"` + `length=255`，填入结构化字段。
