...
| SQLite | 3.x | ✅ 支持（本地文件，无需网络）|
| Apache Doris | 1.2+ | ✅ 支持（MySQL 协议兼容，OLAP）|
| ClickHouse | 22+ | ✅ 支持（HTTP 接口，列存 OLAP）|
| TiDB | 6.0+ | ✅ 支持（MySQL 协议兼容，分布式 HTAP）|

> 完整功能兼容性矩阵见 [docs/database-compatibility.md](./docs/database-compatibility.md)
