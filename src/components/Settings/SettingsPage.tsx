...
export function SettingsPage() {
  ...
  return (
    <div className="flex-1 flex min-w-0 bg-[var(--background-panel)]">
      {/* 左侧导航 */}
      <div className="w-48 flex-shrink-0 bg-[var(--background-base)] border-r border-[var(--border-default)] pt-4">
        <div className="px-4 pb-3 text-xs text-[var(--foreground-muted)] font-medium uppercase tracking-wider">{t('settings.title')}</div>
    ...
    <div className="w-full flex flex-col items-center">
      ...
      <div className="w-full max-w-2xl px-8 pb-8 space-y-4">
        ...
        <div className="flex items-center justify-between">
          <div>
            <p className="text-xs font-medium text-[var(--foreground-default)] mb-1">{t('settings.ghostText')}</p>
          ...
          <button
            ...
            className={`relative w-10 h-5 rounded-full transition-colors flex-shrink-0 ml-4 ${
              ghostTextDefault ? 'bg-[var(--accent)]' : 'bg-[var(--border-strong)]'
            }`}
          >
            <span
    ...
    <div className="w-full max-w-lg p-8 space-y-6">
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="text-xs text-[var(--foreground-muted)] mb-3">{t('settings.languageDesc')}</p>
        ...
        <div>
          <p className="text-xs font-medium text-[var(--foreground-default)] mb-1">{t('settings.tablePageSizeLimit')}</p>
          <p className="text-xs text-[var(--foreground-muted)] mb-3">{t('settings.tablePageSizeLimitDesc')}</p>
...
function PlaceholderSection({ title, description }: { title: string; description: string }) {
  return (
    <div className="w-full max-w-lg p-8 text-center text-[var(--foreground-muted)]">
      <p className="text-sm font-medium text-[var(--foreground-default)] mb-2">{title}</p>
  ...
  return (
    <div className="w-full max-w-lg p-8 space-y-4">
      ...
      <div className="space-y-2 text-xs text-[var(--foreground-muted)]">
        <p><span className="text-[var(--foreground-default)]">{t('settings.aboutVersion')}</span>v{__APP_VERSION__}</p>
        <p><span className="text-[var(--foreground-default)]">{t('settings.techStack')}</span>Tauri 2.x · React 18 · TypeScript · Rust</p>
        <p><span className="text-[var(--foreground-default)]">{t('settings.supportedDb')}</span>MySQL · PostgreSQL · SQLite</p>
      </div>
