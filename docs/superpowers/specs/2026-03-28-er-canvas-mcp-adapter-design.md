...
| 确认机制 | 不走 pending_confirm | ER 画布是设计态，操作可逆，直接生效更流畅。注意：这是对 QueryEditorAdapter/TableFormAdapter 模式的有意偏离，需在代码中注释说明 |
...
```typescript
{
  projectId: number,
  projectName: string,
  connectionId: number | null,   // 从 store.projects 中按 activeProjectId 查找
  tables: [
    {
      id: number,
      name: string,
...
关键：返回业务语义数据（表名、列名、索引、关系），不返回 ReactFlow 的 nodes/edges 渲染细节。`connectionId` 和 `projectName` 从 `store.projects.find(p => p.id === this.projectId)` 获取。
...
[
  ...
  { name: 'update_table', description: '修改表属性', paramsSchema: { tableId: 'number', updates: { name? } } },
  ...
  { name: 'add_index', description: '添加索引', paramsSchema: { tableId: 'number', index: { name, columns: string[], is_unique?, index_type? } } },
  { name: 'update_index', description: '修改索引', paramsSchema: { indexId: 'number', updates: { name?, columns?, is_unique?, index_type? } } },
  { name: 'delete_index', description: '删除索引', paramsSchema: { indexId: 'number', tableId: 'number' } },
...
```
add_table       → store.addTable(name, position)       → emit('er-canvas-reload')  — addTable 直接返回 ErTable
delete_table    → store.deleteTable(tableId)            → emit('er-canvas-reload')
add_column      → store.addColumn(tableId, columnDef)   → emit('er-canvas-reload')  — 见下方 ID 获取策略
update_table    → store.updateTable(tableId, updates)   → emit('er-canvas-reload')
...
add_relation    → store.addRelation(relationDef)         → emit('er-canvas-reload')  — 见下方 ID 获取策略
delete_relation → store.deleteRelation(relationId)       → emit('er-canvas-reload')
add_index       → store.addIndex(tableId, indexDef)      → emit('er-canvas-reload')
update_index    → store.updateIndex(indexId, updates)    → emit('er-canvas-reload')
delete_index    → store.deleteIndex(indexId, tableId)    → emit('er-canvas-reload')
```

每个 CRUD 操作返回 `{ success: true, data: { id: ... } }` 供 AI 追踪实体 ID。

**实体 ID 获取策略**：`addTable` 直接返回 `ErTable` 对象含 id。但 `addColumn`、`addRelation`、`addIndex` 的 store action 返回 `void`（内部已更新 state）。adapter 在调用后通过读取 store 最新 state 找到新增的实体来获取 id：

```typescript
// addColumn 示例：
await store.addColumn(tableId, columnDef)
const cols = useErDesignerStore.getState().columns[tableId] ?? []
const newCol = cols[cols.length - 1]  // 新增列在末尾
return { success: true, data: { columnId: newCol?.id } }
```

### 第二类：文本结果 — 调 store 方法（不直接 invoke），结果返回 AI

```
generate_ddl       → store.generateDDL(projectId, dialect)
                   → return { success: true, data: { ddl, dialect } }

diff_with_database → store.diffWithDatabase(projectId)
                   → return { success: true, data: { diff } }
```

注意：使用 store 方法而非直接 `invoke`，因为 store 方法封装了正确的参数格式（如 `er_generate_ddl` 需要 `{ projectId, options: { dialect, ... } }` 而非 `{ projectId, dialect }`）。
...
- erDesignerStore.ts — 27 个 action 原样复用（不修改 store 来返回实体 ID，adapter 通过读取 state 获取）
...
Step 1: ERCanvasAdapter 重写
  - read() 三种模式（含 indexes）
  - exec() 15 个操作（含 update_table + 3 个 index 操作）
  - patch() 属性替换
