...
impl DataSource for OracleDataSource {
    ...
    /// Native bulk write from MigrationRow using parameterized INSERT with batch execution.
    /// Oracle crate supports batch parameterized inserts with `:1 :2` placeholders.
    ...
    async fn bulk_write_native(
        ...
        #[cfg(feature = "oracle-driver")]
        {
            ...
            let total = tokio::task::spawn_blocking(move || {
                ...
                // Session optimizations for bulk write
                let _ = conn.execute("ALTER SESSION SET SKIP_UNUSABLE_INDEXES = TRUE", &[]);
                let _ = conn.execute("ALTER SESSION SET COMMIT_WRITE = 'BATCH,NOWAIT'", &[]);
                ...
                for row in &rows {
                    ...
                    for val in &row.values {
                        params.push(migration_value_to_oracle_param(val));
                    }
                    batch.add_row(&params);
                }

                let total_executed = stmt.execute_batch(&batch)
                    .map_err(|e| AppError::Datasource(format!("Oracle native batch insert: {}", e)))?;
...
/// Convert a MigrationValue directly to an Oracle-compatible parameter type.
/// Avoids intermediate serde_json::Value conversion.
#[cfg(feature = "oracle-driver")]
fn migration_value_to_oracle_param(val: &crate::migration::native_row::MigrationValue) -> oracle_crate::Param {
    use crate::migration::native_row::MigrationValue;
    match val {
        MigrationValue::Null => oracle_crate::Param::Null,
        MigrationValue::Bool(b) => oracle_crate::Param::from(if *b { 1i32 } else { 0i32 }),
        MigrationValue::Int(i) => oracle_crate::Param::from(*i),
        MigrationValue::UInt(u) => {
            // Oracle NUMBER supports large integers; convert directly
            if *u > i64::MAX as u64 {
                // For very large u64, use string representation
                oracle_crate::Param::from(u.to_string())
            } else {
                oracle_crate::Param::from(*u as i64)
            }
        }
        MigrationValue::Float(f) => oracle_crate::Param::from(*f),
        MigrationValue::Decimal(d) => oracle_crate::Param::from(d.as_str()),
        MigrationValue::Text(s) => oracle_crate::Param::from(s.as_str()),
        MigrationValue::Blob(b) => oracle_crate::Param::from(b.as_slice()),
    }
}
