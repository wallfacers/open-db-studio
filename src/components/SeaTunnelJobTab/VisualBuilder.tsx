...
const inputCls =
  'w-full bg-background-base border border-border-strong rounded px-2.5 py-1.5 text-xs text-foreground-default ' +
  'placeholder-foreground-muted/50 focus:outline-none focus:border-accent/60 transition-colors';

const labelCls = 'text-[10px] font-medium text-foreground-muted uppercase tracking-wide';
...
const FieldInput: React.FC<FieldInputProps> = ({ field, value, onChange, t }) => {
  ...
  if (field.type === 'number') {
    ...
    return (
      <div className="flex items-stretch border border-border-strong rounded overflow-hidden focus-within:border-accent/60 transition-colors">
        <input
          type="number"
          value={value}
          onChange={(e) => onChange(e.target.value)}
          placeholder={field.required ? `${label} ${t('seaTunnelJob.visualBuilder.required')}` : label}
          className="flex-1 min-w-0 bg-background-base px-2.5 py-1.5 text-xs text-foreground-default placeholder-foreground-muted/50 focus:outline-none [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none"
        />
        <div className="flex flex-col border-l border-border-strong bg-background-base">
          <button type="button" onClick={() => onChange(String(numVal + 1))}
            className="flex-1 flex items-center justify-center px-1.5 text-accent hover:text-accent hover:bg-background-elevated transition-colors border-b border-border-strong">
          ...
          <button type="button" onClick={() => onChange(String(Math.max(0, numVal - 1)))}
            className="flex-1 flex items-center justify-center px-1.5 text-accent hover:text-accent hover:bg-background-elevated transition-colors">
...
const ConnectorPanel: React.FC<ConnectorPanelProps> = ({ title, config, onChange, t, scopeId, section }) => {
  ...
  return (
    <div className="flex flex-col gap-3 h-full">
      <div className="text-[11px] font-semibold text-accent uppercase tracking-wider pb-1 border-b border-border-strong">
        {title}
      </div>
      ...
      <div className="flex flex-col gap-2.5 overflow-y-auto flex-1 pr-0.5">
        {fields.length === 0 && (
          <p className="text-xs text-foreground-muted italic mt-2">{t('seaTunnelJob.visualBuilder.noExtraConfig')}</p>
        )}
        {fields.map((field) => (
    ...
    <div className="flex flex-col gap-3 h-full">
      <div className="flex items-center justify-between pb-1 border-b border-border-strong">
        ...
        <button
          ...
          className="text-[10px] text-accent hover:text-foreground px-2 py-0.5 rounded border border-border-strong hover:border-accent/60 transition-colors"
        >
      ...
      <div className="flex flex-col gap-3 overflow-y-auto flex-1 pr-0.5">
        {transforms.length === 0 && (
          <p className="text-xs text-foreground-muted italic mt-2">{t('seaTunnelJob.visualBuilder.noTransformSteps')}</p>
        )}
        {transforms.map((tr, idx) => {
          const fields = TRANSFORM_FIELDS[tr.type] ?? [];
          return (
            <div key={idx} className="bg-background-base border border-border-strong rounded p-2.5 flex flex-col gap-2">
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className="text-foreground-muted hover:text-error transition-colors text-xs px-1"
...
const VisualBuilder: React.FC<VisualBuilderProps> = ({ value, onChange, scopeId }) => {
  ...
  return (
    <div className="flex flex-col h-full gap-0">
      {/* Env bar */}
      <div className="flex items-center gap-3 px-3 py-2 bg-background-base border-b border-border-strong flex-shrink-0">
        <span className={`${labelCls} whitespace-nowrap`}>{t('seaTunnelJob.visualBuilder.parallelism')}</span>
        {/* 自定义数字步进器，避免原生 spinner 样式问题 */}
        <div className={`flex items-stretch border border-border-strong rounded overflow-hidden focus-within:border-accent/60 transition-colors ${parallelismHighlight.className}`} style={{ width: '80px' }}>
          <input
            type="number"
            min={1}
            ...
            className="flex-1 min-w-0 bg-background-base px-2 py-1.5 text-xs text-foreground-default focus:outline-none [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none"
          />
          <div className="flex flex-col border-l border-border-strong bg-background-base">
            <button
              type="button"
      ...
      {/* Three-column layout */}
      <div className="flex flex-1 overflow-hidden divide-x divide-border-strong">
        {/* Source */}
